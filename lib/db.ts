...
const SUBSCRIPTIONS_FILE =
  process.env.SUBBED_DB_FILE || path.resolve(process.cwd(), 'data', 'subscriptions.json');
...
function convexToLocalSubscription(convexSub: {
  channelId: string;
  channelName: string;
  channelUrl: string;
  createdAt: string;
...
export async function syncSubscriptions() {
  ...
  try {
    ...
    for (const localSub of localSubs) {
      const convexSub = convexSubs.find(
        (cs: { channelId: string }) => cs.channelId === localSub.id
      );
      if (!convexSub) {
