...
export const AdvancedVideoFeed: React.FC<AdvancedVideoFeedProps> = React.memo(
  ({ className = '' }) => {
    ...
    const { query, debouncedQuery, updateQuery, clearQuery, isSearching } = useDebouncedSearch(
      '',
      () => {
          ...
          <CardContent className="p-4">
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              >
                ...
                <Badge
                  variant="secondary"
                  className="ml-2 backdrop-blur-sm bg-primary/10 border-primary/20"
                >
                    ...
                    <SelectItem key={sub.id} value={sub.id}>
                      <div className="flex items-center gap-2 max-w-[200px]">
                        ...
                        <span className="truncate font-medium">
                          {sub.title || `Channel ${sub.id.slice(-8)}`}
                        </span>
                      </div>
                    </SelectItem>
                  ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={feedType} onValueChange={handleFeedTypeChange}>
                      <SelectTrigger
                        id="content-type"
                        className="backdrop-blur-sm bg-white/60 dark:bg-gray-800/60 border-white/30"
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="backdrop-blur-md bg-white/90 dark:bg-gray-900/90 border-white/20">
                  ...
                  <div className="space-y-2">
                    ...
                    <div
                      className="text-sm text-muted-foreground p-2 backdrop-blur-sm bg-white/40 dark:bg-gray-800/40 border border-white/20 rounded"
                      aria-live="polite"
                    >
                      <span className="font-semibold">{filteredAndSortedFeed.length}</span> video
                      {filteredAndSortedFeed.length !== 1 ? 's' : ''}
                      {debouncedQuery && (
                        <>
                          <br />
            ...
            <CardContent className="p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {feedType !== 'all' && (
                  <Badge
                    variant="secondary"
                    className="backdrop-blur-sm bg-primary/10 border-primary/20"
                  >
