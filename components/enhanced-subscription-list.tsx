...
export const EnhancedSubscriptionList: React.FC<SubscriptionListProps> = React.memo(
  ({ subs, selected, loading, onSelect, onRefreshAll, onRemove, onOpenClearConfirm }) => {
    ...
    const filteredAndSortedSubs = useMemo(() => {
      const filtered = subs.filter(sub => {
        ...
        return sub.title?.toLowerCase().includes(query) || sub.id.toLowerCase().includes(query);
      });
    ...
    const groupedSubs = useMemo(() => {
      ...
      if (groupBy === 'date') {
        ...
        const recent = filteredAndSortedSubs.filter(
          sub => new Date(sub.created_at || 0) >= thisWeek
        );
        const thisMonthSubs = filteredAndSortedSubs.filter(
          sub =>
            new Date(sub.created_at || 0) >= thisMonth && new Date(sub.created_at || 0) < thisWeek
        );
        const older = filteredAndSortedSubs.filter(
          sub => new Date(sub.created_at || 0) < thisMonth
        );
    ...
    const SubscriptionItem: React.FC<{ sub: Sub; isSelected: boolean }> = React.memo(
      ({ sub, isSelected }) => (
        <li
                ...
                <div key={group.title} className="space-y-2 sm:space-y-3">
                  ...
                  {(groupBy === 'none' || group.isExpanded) && (
                    <ul id={`group-${group.title}`} className="space-y-2 sm:space-y-3">
                      {group.subs.map(sub => (
                        <SubscriptionItem key={sub.id} sub={sub} isSelected={selected === sub.id} />
                      ))}
                    </ul>
                  )}
                </div>
