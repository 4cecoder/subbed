...
export const EnhancedVirtualList: React.FC<EnhancedVirtualListProps> = React.memo(({
  ...
  const visibleRange = useMemo(() => {
    ...
    while (end < items.length && currentPos < viewportEnd + (overscan * 100)) { // Buffer based on overscan
      end++;
        ...
        <div className="flex items-center justify-center h-full">
          <div className="text-center space-y-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Try adjusting your search or refresh the feed</p>
            </div>
          </div>
        </div>
      )}
